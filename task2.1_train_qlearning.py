# task2.1_train_qlearning.py (derived from task2.1_train_sarsa.py)
...
def main():
    print("Starting Q-Learning Agent training pipeline for multiple environments...")
    ...
    for env_name, env_id_val in environments_to_run.items():
        ...
        # --- Agent Setup ---
        q_learning_agent = agents.QLearningAgent( # Changed from SARSAAgent
            action_space=env.action_space, 
            params=config.Q_LEARNING_AGENT_PARAMS # Changed from SARSA_AGENT_PARAMS
        )

        # --- Task Setup ---
        task = commons.AbstractRLTask(env, q_learning_agent) # Use the new agent

        # --- Training ---
        print(f"Training {q_learning_agent.id} on {env_name} for {num_train_episodes} episodes.")
        if viz_in_train:
        ...
        print(f"Training on {env_name} finished. Final epsilon for {q_learning_agent.id}: {q_learning_agent.epsilon:.4f}")

        # --- Plotting Average Returns ---
        plt.figure(figsize=(12, 6))
        plt.plot(avg_returns_train, label=f'{q_learning_agent.id} Average Returns on {env_name}')
        plt.title(f'Q-Learning Agent Training on {env_name}') # Changed title
        plt.xlabel('Episode')
        plt.ylabel('Average Return (Smoothed)')
        plt.legend()
        plt.grid(True)
        
        output_dir = f"results/2.1_qlearning/{env_name}/" # Changed output directory
        os.makedirs(output_dir, exist_ok=True)
        plot_filename = os.path.join(output_dir, f"average_returns_qlearning_{env_name}.png") # Changed plot filename
        ...
        if num_visualize_episodes_post > 0:
            print(f"\nVisualizing (post-training) {q_learning_agent.id} on {env_name} for {num_visualize_episodes_post} episode(s), up to {max_steps_visualize_post} steps each...")
    ...
    print("\nQ-Learning agent training pipeline complete for all specified environments.")
